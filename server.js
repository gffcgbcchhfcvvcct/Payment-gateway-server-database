...
app.post('/api/verification/upload', authMiddleware, upload.single('file'), async (req, res) => {
  /* Guard: ensure profile row exists before inserting into verification_documents.
     verification_documents.user_id has a FK → profiles.id, so if the profile was
     never saved (e.g. /api/auth/profile failed silently at signup) the insert will
     throw a foreign key constraint error. Return a clear message instead. */
  ...
  if (!profile) {
    return res.status(400).json({
      error: 'Account setup incomplete. Your profile was not found. Please complete registration before uploading documents.'
    });
  }
...
app.post('/api/verification/submit', authMiddleware, async (req, res) => {
  /* Guard: same profile check as the upload endpoint */
  ...
  if (!profileCheck) {
    return res.status(400).json({
      error: 'Account setup incomplete. Your profile was not found. Please complete registration before submitting verification.'
    });
  }
